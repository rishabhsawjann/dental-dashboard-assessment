...
function EditProfileModal({ open, onClose, patient, onSave }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!form.contact.trim() || !form.dob.trim()) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative animate-fadeInUp transform transition-all duration-300 hover:scale-[1.02]">
        ...
        <p className="text-gray-600 mb-6">Would you like to receive appointment and update notifications via email?</p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative animate-fadeInUp transform transition-all duration-300 hover:scale-[1.02]">
        ...
        <p className="text-gray-600 mb-6">
          We take your privacy seriously. All your personal and medical data is securely stored and protected with industry-standard encryption. We never share your information with third parties without your explicit consent. Your trust and safety are our top priorities.
        </p>
        {accepted ? (
...
export default function PatientSettingsPage() {
  ...
  const { patients, incidents, updatePatient } = useAppData();
  const [showLogoutModal, setShowLogoutModal] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const [showNotificationModal, setShowNotificationModal] = useState(false);
  const [showPrivacyModal, setShowPrivacyModal] = useState(false);
  const [emailSubscribed, setEmailSubscribed] = useState(() => {
    const stored = localStorage.getItem('emailSubscribed');
  ...
  const patientIncidents = incidents.filter(i => i.patientId === currentPatient?.id);
  const completedAppointments = patientIncidents.filter(i => i.status === 'Completed');
  const memberSince = patientIncidents.length > 0
    ? formatDate(patientIncidents.map(i => i.appointmentDate).sort()[0])
    : (currentPatient?.dob ? formatDate(currentPatient.dob) : '-');

  
  const info = [
    {
      icon: <Mail className="w-5 h-5 text-blue-600" />, label: 'Email', value: user?.email || '-',
    },
    {
      icon: <Phone className="w-5 h-5 text-blue-600" />, label: 'Phone', value: currentPatient?.contact || '-',
    },
    {
      icon: <CalendarIcon className="w-5 h-5 text-blue-600" />, label: 'Date of Birth', value: currentPatient?.dob ? formatDate(currentPatient.dob) : '-',
    },
  ];
  const account = [
    {
      icon: <MapPin className="w-5 h-5 text-blue-600" />, label: 'Address', value: currentPatient?.address || 'Not Provided',
    },
    {
      icon: <User2 className="w-5 h-5 text-blue-600" />, label: 'Appointments', value: patientIncidents.length,
    },
    {
  ...
  function handleNotificationChange(val) {
    ...
    localStorage.setItem('emailSubscribed', JSON.stringify(val));
  }

  function handlePrivacyAccept() {
    setPrivacyAccepted(true);
    localStorage.setItem('privacyAccepted', JSON.stringify(true));
  }
